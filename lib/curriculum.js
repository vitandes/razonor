...
// La IA solo aparece como tema explícito cuando realmente pertenece a la
// historia. En los demás capítulos, evaluar una respuesta se presenta como
// análisis de información o detección de errores.
export const AI_CONTEXT_CHAPTERS = new Set([2, 7, 8, 9, 10, 14, 15, 39, 65]);

function hasAiReference(value) {
  return /\bIA\b|inteligencia artificial|Razobot|asistente virtual/i.test(String(value || ""));
}

function neutralizeAiLanguage(value) {
  return String(value || "")
    .replace(/hech([oa]) por (?:una )?IA/gi, "editad$1 digitalmente")
    .replace(/generad([oa]) por (?:una )?IA/gi, "cread$1 digitalmente")
    .replace(/la IA del villano/gi, "el sistema del villano")
    .replace(/la IA del futuro/gi, "el sistema del futuro")
    .replace(/la IA de la casa/gi, "el sistema de la casa")
    .replace(/inteligencia artificial/gi, "sistema automático")
    .replace(/\bRazobot\b/g, "la fuente")
    .replace(/\bLas IA\b/g, "Las fuentes")
    .replace(/\blas IA\b/g, "las fuentes")
    .replace(/\bLa IA\b/g, "La fuente")
    .replace(/\bla IA\b/g, "la fuente")
    .replace(/\bUna IA\b/g, "Una fuente")
    .replace(/\buna IA\b/g, "una fuente")
    .replace(/\bIA\b/g, "fuente")
    .replace(/\basistente virtual\b/gi, "sitio consultado")
    .replace(/de una fuente inventando/gi, "de una respuesta inventada")
    .replace(/Sí, es la fuente analizando/gi, "Sí, el análisis es suficiente")
    .replace(/la fuente sabe/gi, "la recomendación basta")
    .replace(/^la fuente/, "La fuente");
}

function neutralMechanicFor(reto = {}) {
  const decision = /qué haces|qué respondes|qué le dices|aceptas|confías|es la mejor solución|qué eliges/i.test(
    reto.question || "",
  );
  return decision ? "comprension" : "error";
}

function neutralizeIaReto(reto, caseTitle) {
  const mechanic = neutralMechanicFor(reto);
  const originalQuestion = reto.question || "";
  const assertion = neutralizeAiLanguage(reto.aiSays);
  const question = hasAiReference(originalQuestion)
    ? mechanic === "error"
      ? "¿La afirmación está suficientemente comprobada?"
      : "¿Qué decisión está mejor sustentada?"
    : neutralizeAiLanguage(originalQuestion);

  reto.mechanic = mechanic;
  reto.prompt =
    mechanic === "error"
      ? `En “${caseTitle}”, una conclusión parece segura. Revisa si contiene un error.`
      : `En “${caseTitle}”, analiza la información antes de tomar una decisión.`;
  reto.clues = [
    ...(reto.clues || []).map(neutralizeAiLanguage),
    `Afirmación para comprobar: ${assertion}`,
  ];
  reto.question = question;
  reto.options = (reto.options || []).map(neutralizeAiLanguage);
  reto.answer = neutralizeAiLanguage(reto.answer);
  reto.hint = neutralizeAiLanguage(reto.hint);
  reto.explicacion = neutralizeAiLanguage(reto.explicacion);
  delete reto.aiSays;
  return reto;
}
    ...
    for (const caseData of chapter.cases || []) {
      if (!AI_CONTEXT_CHAPTERS.has(Number(chapter.id))) {
        for (const reto of caseData.retos || []) {
          if (reto.mechanic === "ia") neutralizeIaReto(reto, caseData.title);
        }
      }
    }
